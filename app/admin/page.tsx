...
export default function AdminDashboard() {
  ...
  const processContent = (data: any, parentKey?: string): any => {
    ...
    if (typeof data === 'object') {
      ...
      // Hero defaults — only style/meta properties, no image padding
      if (newData.hero && typeof newData.hero === 'object') {
        // Preserve only admin-uploaded images; ensure the array exists but don't inject external URLs
        if (!Array.isArray(newData.hero.images)) {
          newData.hero.images = [];
        }
      ...
      if (Array.isArray(newData.trustpilotReviews)) {
        newData.trustpilotReviews = {
          title: 'Excellent on Trustpilot',
          rating: 5,
          reviews: newData.trustpilotReviews
        };
      } else if (newData.trustpilotReviews === undefined && !parentKey && newData.hero) {
         newData.trustpilotReviews = {
           title: 'Excellent on Trustpilot',
           rating: 5,
           reviews: [
